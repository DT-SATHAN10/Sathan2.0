...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        ...
        await axios.get('https://gist.github.com/DT-SATHAN10/571cbea6d15f48be51559198f7da9e2d/raw/').then(async (ann) => {
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (config.FULLEVA == 'true') {
                   ...
                   await conn.sendMessage(conn.user.jid, fs.readFileSync("./image/logo.jpg"), MessageType.image, { caption: `*ʜᴇʏ  Sathan2.0 ᴡᴏʀᴋɪɴɢ ᴜs ᴘᴜʙʟɪᴄ* \n\n*⚠ #ownercmnd ғᴏʀ ᴏᴡɴᴇʀ ᴄᴏᴍᴍᴀɴᴅs* \n\n*Thank You For Using Sathan2.0 ❤‍🩹*`});
               }               
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
...
*/
                       ...
                       try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '  *Sathan2.0*  ' +
